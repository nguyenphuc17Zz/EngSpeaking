...
import { PRESET_SCENARIOS, type PracticeScenario } from "@/lib/voice/scenarios";
...
interface VoiceSessionWorkspaceProps {
  initialScenarioId?: string;
}

export function VoiceSessionWorkspace({ initialScenarioId }: VoiceSessionWorkspaceProps) {
  ...
  const initialScenario = initialScenarioId
    ? PRESET_SCENARIOS.find((s) => s.id === initialScenarioId) || PRESET_SCENARIOS[0]
    : PRESET_SCENARIOS[0];

  // ─── Mode & Scenario State ───────────────────────────────────────────
  const [sessionMode, setSessionMode] = useState<"goal" | "free">("goal");
  const [selectedScenario, setSelectedScenario] = useState<PracticeScenario>(initialScenario);

  // Sync with prop when URL changes
  useEffect(() => {
    if (initialScenarioId) {
      const match = PRESET_SCENARIOS.find((s) => s.id === initialScenarioId);
      if (match && match.id !== selectedScenario.id) {
  ...
  }, [status, handleBargeIn, handleStartSpeaking]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const startSession = useCallback(
    async (scenario?: PracticeScenario, mode: "goal" | "free" = sessionMode) => {
      ...
      try {
        ...
        setDynamicHints({
          tier1Keywords: [
            { term: "hit the ground running", meaning: "Bắt đầu đầy năng lượng" },
            { term: "from my perspective", meaning: "Theo góc nhìn của tôi" },
          ],
          tier2Starters: [
            { starter: "When working on this project, our primary goal was to...", meaning: "Mục tiêu chính là..." },
          ],
          tier3FullAnswer: {
            en: "I would be happy to share my experience working on this project.",
            vi: "Tôi rất vui được chia sẻ kinh nghiệm làm dự án này.",
          },
          directStarter: "I would be happy to share that...",
        });
      } finally {
  ...
  const handleStopAndProcess = useCallback(async () => {
    ...
    try {
      ...
      if (!spokenText) {
        toast.info("Chưa phát hiện giọng nói", "Vui lòng giữ phím Space hoặc nhấn micro để nói lại.");
        setStatus("listening");
      ...
      const durationMs = Math.max(500, now - speechStartMs);
      const turnWpm = calculateSpeechRateWpm(spokenText, durationMs);
      const turnTtr = calculateTypeTokenRatio(spokenText);

      const nextTurnNumber = sessionStats.totalTurns + 1;
      const currentDiscourse = getDiscourseStage(nextTurnNumber, selectedScenario.targetTurns);

      let currentActiveTwist = activeTwist;
      if (!currentActiveTwist && sessionMode === "goal") {
        const potentialTwist = getScenarioTwist(selectedScenario.id, nextTurnNumber);
        if (potentialTwist) {
          currentActiveTwist = potentialTwist;
          setActiveTwist(potentialTwist);
          soundEffects.playAIReady();
          toast.warning(`⚡ Biến cố: ${potentialTwist.titleVi}`, potentialTwist.promptAiVi);
        }
      }

      let isTwistResolved = false;
      if (currentActiveTwist && !currentActiveTwist.isResolved) {
        const resolution = evaluateTwistResolution(currentActiveTwist, spokenText);
        if (resolution.isResolved) {
          isTwistResolved = true;
          currentActiveTwist = { ...currentActiveTwist, isResolved: true };
          setActiveTwist(currentActiveTwist);
          setActiveTwistFeedback(resolution.feedbackVi);
      ...
      const userTurn: ConversationTurn = {
        ...
        pedagogy: {
          ...
          turnScore: 85,
          nativeReformulation: spokenText,
        },
      };
      ...
      const scenarioContext =
        sessionMode === "goal"
          ? `Scenario: ${selectedScenario.title}. Goal: ${selectedScenario.goal}. AI Role: ${selectedScenario.aiRole}. User Role: ${selectedScenario.userRole}. Tactical Tone: ${selectedScenario.tacticalGuide?.recommendedTone || "Professional"}. Pitfalls to watch: ${selectedScenario.tacticalGuide?.pitfallsToAvoid || "None"}.`
          : "Free-flow casual speaking practice. Greet warmly and maintain lively conversational momentum.";
      ...
      const errorMsg = err instanceof Error ? err.message : "Đã xảy ra lỗi kết nối";
      toast.error("Lỗi đàm thoại", errorMsg);
      setStatus("listening");
    } finally {
      setIsProcessing(false);
    }
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] bg-background select-none overflow-hidden">
      ...
      <main className="flex-1 p-2.5 sm:p-3 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-2.5 sm:gap-3 min-h-0">
            ...
            <div className="px-3.5 py-2.5 border-b border-border/60 bg-secondary/40 flex items-center justify-between gap-2 shrink-0">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xs text-foreground truncate">{selectedScenario.title}</span>
                  <Badge variant="outline" className="text-[10px] font-mono shrink-0">
                    {DISCOURSE_STAGES.find((s) => s.key === getDiscourseStage(sessionStats.totalTurns + 1, selectedScenario.targetTurns))?.icon}{" "}
                    {DISCOURSE_STAGES.find((s) => s.key === getDiscourseStage(sessionStats.totalTurns + 1, selectedScenario.targetTurns))?.labelVi}
                  </Badge>
                </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                >
                  ...
                  <span className="hidden sm:inline">AI Kịch Bản</span>
                </Button>
              </div>
            </div>
          )}

          {/* Active Twist Challenge Banner */}
          {activeTwist && !activeTwist.isResolved && (
            <div className="mx-3.5 mt-2.5 p-2.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-2.5 shrink-0 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
                ...
                <div className="space-y-1 max-w-xs">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Nhấn nút bắt đầu để mở đầu kịch bản &ldquo;{selectedScenario.titleVi}&rdquo; và cùng đàm thoại theo nhịp tự nhiên.
                  </p>
                </div>
                <Button
        ...
        <div className="lg:col-span-5 h-full flex flex-col gap-2.5 sm:gap-3 min-h-0 overflow-hidden">
          ...
          {/* Quick Hints Scaffolding Embedded Panel */}
