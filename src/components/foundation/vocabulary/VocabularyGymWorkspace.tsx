...
import { ArrowLeft, BookOpen, Sparkles, RotateCcw, ChevronDown } from "lucide-react";
...
interface VocabularyGymWorkspaceProps {
  initialWord?: string;
}

export function VocabularyGymWorkspace({ initialWord }: VocabularyGymWorkspaceProps) {
  ...
  // Auto-search initialWord when provided via URL
  useEffect(() => {
    if (initialWord) {
      const clean = decodeURIComponent(initialWord).trim().toLowerCase();
      if (clean && currentWord?.word?.toLowerCase() !== clean) {
        searchWord(clean);
      }
    }
  ...
  return (
    <div className="w-full h-full max-h-[calc(100vh-5.5rem)] bg-card text-foreground flex flex-col overflow-hidden rounded-3xl border border-border/80 shadow-xs">
      ...
      <header className="h-13 border-b border-border/60 px-4 sm:px-6 flex items-center justify-between bg-card/60 backdrop-blur-md shrink-0 gap-3">
        ...
        {/* Right: Step Toggle + Filters + Next Word + Search + GlobalAiSelector */}
        <div className="flex items-center gap-2 shrink-0">
          ...
          <VocabularyCommandSearch
            ...
            onSearch={(q) => {
              searchWord(q);
              router.push(`/vocabulary/${encodeURIComponent(q.trim().toLowerCase())}`);
            }}
            onSelectWord={(w) => {
              selectWord(w);
              router.push(`/vocabulary/${encodeURIComponent(w.word.toLowerCase())}`);
            }}
      ...
      <main className="flex-1 min-h-0 p-2.5 sm:p-3.5 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-2.5 lg:gap-3">
        ...
        {/* Column 3 (3 cols): Interactive Voice Studio OR Feedback Card */}
