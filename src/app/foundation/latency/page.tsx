...
export default function LatencyTrainingPage() {
  ...
  const handleStopRecord = useCallback(async () => {
    if (recorder.status !== "recording" || !currentTask) return;

    soundEffects.playMicStop();
    speechRec.stopListening();

    const tSpeech = speechStartTimestamp || Date.now();
    const measuredLatencyMs = Math.max(400, tSpeech - promptDisplayTime);
    const measuredDurationMs = Math.max(800, recordingDurationMs);
    ...
    try {
      ...
      // Read active provider & model from settings-store
      let provider = "gemini";
      let model = "auto";
      try {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Enter" && lastEvaluation) {
        e.preventDefault();
        handleContinueTask();
      }
